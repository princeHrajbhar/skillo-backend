  ...
  try {
    ...
    if (files?.instructorImage?.[0] && validatedData.instructor) {
      if (existingProgram.instructor?.image?.publicId) {
        oldFiles.push({ type: 'instructorImage', publicId: existingProgram.instructor.image.publicId });
      }
    ...
    if (uploadedFiles.length > 0) {
      const deletionPromises = uploadedFiles.map(file => 
        deleteFile(file.publicId).catch(err => 
          console.error(`Failed to delete newly uploaded ${file.type} with publicId ${file.publicId}:`, err)
        )
      );
      await Promise.allSettled(deletionPromises);
    }
